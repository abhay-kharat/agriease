...
public class DeliveryService {
    ...
    public OrderStatusResponseDto confirmOrder(Long orderId, String supplierEmail) {
        ...
        userRepository.findByEmail(agent.getEmail()).ifPresent(user -> {
            notificationService.createNotification(
                ...
                "You have been assigned to deliver Order #" + order.getDisplayOrderNumber() + ".",
                "DELIVERY"
            );
        });
        ...
        if (order.getFarmer() != null) {
            notificationService.createNotification(
                ...
                "Order Confirmed & Out for Agent",
                "Your order (Order #" + order.getDisplayOrderNumber() + ") has been confirmed and a delivery agent (" + agent.getName() + ") is assigned.",
                "ORDER"
            );
        }
    ...
    public OrderStatusResponseDto assignOrderToAgent(Long orderId, Long agentId, String supplierEmail) {
        ...
        if (assigningDifferentAgent) {
            // Notify new Delivery Agent
            userRepository.findByEmail(agent.getEmail()).ifPresent(user -> {
                notificationService.createNotification(
                    user.getId(),
                    "New Delivery Assigned",
            ...
            // Notify Farmer about new agent
            if (order.getFarmer() != null) {
                notificationService.createNotification(
                    order.getFarmer().getId(),
                    "Agent Re-assigned",
                    "A new delivery agent (" + agent.getName() + ") has been assigned to your order (Order #" + order.getDisplayOrderNumber() + ").",
                    "ORDER"
                );
            }
        }
    ...
    public OrderStatusResponseDto acceptOrder(Long orderId, String agentEmail, String location) {
        ...
        if (order.getFarmer() != null) {
            notificationService.createNotification(
                ...
                "A delivery agent has accepted your order (Order #" + order.getDisplayOrderNumber() + ") and will prepare it for pickup.",
                "DELIVERY"
            );
        }
        ...
        if (order.getSupplier() != null) {
            notificationService.createNotification(
                ...
                "Agent Accepted Order",
                "Delivery agent has accepted the pickup for Order #" + order.getDisplayOrderNumber() + ".",
                "DELIVERY"
            );
        }
    ...
    public OrderStatusResponseDto rejectOrder(Long orderId, String agentEmail, String location, String reason) {
        ...
        if (order.getSupplier() != null) {
            notificationService.createNotification(
                order.getSupplier().getId(),
                "Order Rejected by Agent",
                "Delivery agent rejected the order (Order #" + order.getDisplayOrderNumber() + ").",
                "DELIVERY"
            );
        }
    ...
    private OrderStatusResponseDto updateByAgent(Long orderId, Long agentId, OrderStatus targetStatus, String location, String proofUrl) {
        ...
        if (targetStatus == OrderStatus.PICKED_UP) {
            if (order.getFarmer() != null) {
                notificationService.createNotification(order.getFarmer().getId(), "Order Picked Up", "Your order (Order #" + order.getDisplayOrderNumber() + ") has been picked up by the delivery agent.", "DELIVERY");
            }
            if (order.getSupplier() != null) {
                notificationService.createNotification(order.getSupplier().getId(), "Order Picked Up", "Delivery agent picked up Order #" + order.getDisplayOrderNumber() + ".", "DELIVERY");
            }
        } else if (targetStatus == OrderStatus.OUT_FOR_DELIVERY) {
            if (order.getFarmer() != null) {
                notificationService.createNotification(order.getFarmer().getId(), "Out for Delivery", "Your order (Order #" + order.getDisplayOrderNumber() + ") is out for delivery and will reach you soon.", "DELIVERY");
            }
        } else if (targetStatus == OrderStatus.DELIVERED) {
            if (order.getFarmer() != null) {
                notificationService.createNotification(order.getFarmer().getId(), "Order Delivered", "Your order (Order #" + order.getDisplayOrderNumber() + ") has been delivered successfully!", "DELIVERY");
            }
            if (order.getSupplier() != null) {
                notificationService.createNotification(order.getSupplier().getId(), "Order Delivered", "Order #" + order.getDisplayOrderNumber() + " has been delivered to the farmer.", "DELIVERY");
            }
        }
