...
import { getStoredMeasurementUnit } from "../../utils/userPreferences";
...
export default function LandMeasurement() {
  ...
  useEffect(() => {
    const handleStorageChange = (event) => {
      if (event.key === "agriease-measurement-unit") {
        setMeasurementUnit(getStoredMeasurementUnit());
      }
    };
  ...
  const displayedArea = useMemo(
    () => (measurementUnit === "metric" ? areaSquareMeters * SQM_TO_HECTARE : areaAcres),
    [areaSquareMeters, areaAcres, measurementUnit]
  );
  const displayedUnitLabel = measurementUnit === "metric" ? "ha" : t("farmer.landMeasurement.acres");
  ...
  return (
    <div className="smart-page">
      ...
      <section className="widget-card smart-form">
        ...
        <p>
          {t("farmer.landMeasurement.points")}: <strong>{points.length}</strong> | {t("farmer.landMeasurement.area")}: <strong>{displayedArea.toFixed(3)} {displayedUnitLabel}</strong>
        </p>
              ...
              <div className="smart-row" key={item.id}>
                ...
                <span>
                  {measurementUnit === "metric"
                    ? (Number(item.areaAcres || 0) * ACRE_TO_HECTARE).toFixed(3)
                    : Number(item.areaAcres || 0).toFixed(3)}{" "}
                  {displayedUnitLabel}
                </span>
